// Runtime selector for this plugin's TypeScript. `hooks.json` runs this file
// with plain `node`, because that is the one interpreter every Claude Code
// install is known to have; what it starts is TypeScript, so something has to
// decide what executes it. This file stays JavaScript, in syntax every Node
// release parses, so that the "your Node is too old" line can be printed by
// the Node that is too old.
//
// Loading this file launches: there is no entry guard to get wrong, and
// nothing imports it. `select-runtime.mjs` is where the choice itself lives,
// for a test that wants it without a run of its own.
//
// Arguments: `--data <dir>`, the entry's path relative to the plugin
// directory, then the entry's own arguments, forwarded untouched.
// `<dir>/.runtime` picks the interpreter: `bun` or `node` force one, and no
// file at all prefers bun and falls back to Node.
import { spawnSync } from "node:child_process";
import { readFileSync } from "node:fs";
import { dirname, join } from "node:path";
import process from "node:process";
import { fileURLToPath } from "node:url";
import { selectRuntime } from "./select-runtime.mjs";

/**
 * libuv resolves `bun.exe` through PATH without a shell, so this is the same
 * probe on every platform.
 *
 * @returns {boolean}
 */
function probeBun() {
	const probe = spawnSync("bun", ["--version"], { stdio: "ignore" });

	return !probe.error && probe.status === 0;
}

/**
 * @param {string} file
 * @returns {string}
 */
function requestedRuntime(file) {
	try {
		return readFileSync(file, "utf8").trim();
	} catch {
		// Unwritten, or unreadable and therefore saying nothing: either way the
		// user has expressed no preference.
		return "";
	}
}

/**
 * The one line a launch that never happened has for the user. Claude Code
 * renders a `systemMessage` as `<Event> says: <text>` for a hook it waits on,
 * and takes the output at all only from a run that exited 0 with one JSON
 * object on stdout. A registration marked `"async": true` is one it does not
 * wait on and shows the user nothing from, so a refusal of that launch is a
 * line nobody reads. Nothing here reads stdin, so the event this run is for is
 * unknown, and `hookSpecificOutput` is addressed to one. A `PreToolUse` run
 * writes no decision this way, so the tool call it was called for goes ahead.
 *
 * Written on the way out rather than followed by `process.exit`, which can
 * truncate a piped stdout before it has flushed: every caller returns on it,
 * and `main` returning leaves the status at the 0 this line needs.
 *
 * @param {string} message
 * @returns {void}
 */
function fail(message) {
	process.stdout.write(JSON.stringify({ systemMessage: message }));
}

/**
 * The entry is named relative to the plugin directory, which is two above the
 * `lib/shared` this launcher is copied into: what a plugin runs this way is
 * not all hooks. The interpreter is handed its own stdio so the entry talks to
 * Claude Code directly. The `--disable-warning` alias survives into Node 26, and
 * stripping is on by default from 22.18, so the pair is a no-op there and the
 * floor stays at 22.6 for free.
 *
 * Bun is told never to install. With no `node_modules` in the working
 * directory or above it, which is most projects a hook runs in, it would
 * otherwise fetch a missing import from the network as the entry runs, and a
 * dependency the plugin lacks is a fault to report, not a download.
 *
 * @param {"bun" | "node"} kind
 * @param {string} name
 * @param {readonly string[]} args
 */
function spawnEntry(kind, name, args) {
	const plugin = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
	const entry = join(plugin, `${name}.mts`);

	return kind === "bun"
		? spawnSync("bun", ["--no-install", entry, ...args], { stdio: "inherit" })
		: spawnSync(
				process.execPath,
				[
					"--experimental-strip-types",
					"--disable-warning=ExperimentalWarning",
					entry,
					...args,
				],
				{ stdio: "inherit" },
			);
}

function main() {
	const argv = process.argv.slice(2);

	if (argv[0] !== "--data" || argv.length < 3) {
		return fail(
			"launch: usage: launch.mjs --data <plugin data dir> <entry> [arguments]",
		);
	}

	const data = String(argv[1]);
	const name = String(argv[2]);

	// `hooks.json` writes `--data "${CLAUDE_PLUGIN_DATA}"`, so an unset variable
	// arrives as an empty argument. Every path built from it would then point at
	// a filesystem root, where a missing config reads as "nothing configured"
	// and the run says nothing at all.
	if (data.trim() === "") {
		return fail(
			`${name}: --data was empty, so CLAUDE_PLUGIN_DATA is unset and the plugin's data directory is unknown.`,
		);
	}

	const runtimeFile = join(data, ".runtime");
	const requested = requestedRuntime(runtimeFile);
	// A pin answers the probe's question, so neither pin spawns anything; a
	// pinned bun that is missing surfaces when the entry itself fails to start.
	const bunFound = requested === "" ? probeBun() : requested === "bun";
	const selected = selectRuntime(
		requested,
		bunFound,
		process.versions.node,
		runtimeFile,
	);

	if (selected.kind === "error") {
		return fail(`${name}: ${selected.message}`);
	}

	const run = spawnEntry(selected.kind, name, argv.slice(3));

	if (run.error) {
		const pinnedAndMissing = requested === "bun" && run.error.code === "ENOENT";

		return fail(
			pinnedAndMissing
				? `${name}: ${runtimeFile} says bun, but no bun was found on PATH.`
				: `${name}: could not start ${selected.kind}: ${run.error.message}`,
		);
	}

	// An entry killed by a signal has no exit code; treat that as a failure
	// rather than as a silent success.
	process.exit(run.status === null ? 1 : run.status);
}

main();
