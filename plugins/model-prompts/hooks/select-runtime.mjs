// Which interpreter runs a hook. Its own module because `launch.mjs` launches
// as soon as it is loaded, and this choice is worth testing without spawning
// anything. JavaScript, in syntax every Node release parses, so the "your Node
// is too old" message can be composed by the Node that is too old.
//
// Node gained `--experimental-strip-types` in 22.6. Below that there is no way
// to run a hook, and the launcher's one stderr line is the whole user
...
/**
 * Which interpreter runs the hook, given what `.runtime` asked for, whether a
