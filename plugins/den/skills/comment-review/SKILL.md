---
name: comment-review
description: Launch the comment-reviewer on the pending diff, with the diff already in its first message when it fits inline. Invoke once functional churn has settled. The argument is a git diff range and nothing else (revisions, optionally `-- paths`), never a description of the scope; omit it to review the working tree against HEAD.
