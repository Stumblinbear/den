---
name: implementer
description: Executes a brief, comes back with a question when what it finds changes what should be built, and declares the choices it makes. Runs on opus; the fable override is for work where the correctness argument is a derivation and a wrong result passes green, and needs the user's explicit per-task approval.
