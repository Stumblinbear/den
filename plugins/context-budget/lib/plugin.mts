...
// The entries share one record and one configuration file read through one
// parser, so the first of them to meet a fault of the file or the parser
// silences the rest, and a prompt run of the measurement hook that works again
// takes that fault back, whichever entry said it. What they do not share is
// what one entry's own run coming apart costs, which is a policy per entry
// below.
