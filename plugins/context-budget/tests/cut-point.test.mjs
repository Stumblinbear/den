...
// saying something useful when it cannot, since its output is read as prose by
// an agent that has no other way to tell what went wrong.
...
// The script reads the session id from its own environment, as the skill's
// preamble shell hands it over; `env` here is what that shell would carry.
const run = (args, env = {}) =>
...
test("lists the cached cut points oldest first, with what each one summarizes and keeps", () => {
  ...
  assert.match(out, /Prompt cache, read at \d\d:\d\d \(1h lifetime\)\. Cached prompts, oldest first:/);
  assert.match(
    out,
    new RegExp(
      `1\\. "Read the brief and start on the scanner"\\s+` +
        `sent ${hhmm(opened)} \\| valid until ${hhmm(opened, HOUR)} \\| 100K tokens before it, keeps 100K\\s+` +
        `2\\. "Now add the skill that takes a fresh reading"\\s+` +
        `sent ${hhmm(started)} \\| valid until ${hhmm(started, HOUR)} \\| 150K tokens before it, keeps 50K`,
    ),
...
test("the list is three prompts spread across the cached context, not across the clock", () => {
  ...
  assert.match(out, /"Prompt number 3"\s+sent \d\d:\d\d \| valid until \d\d:\d\d \| 140K tokens before it, keeps 60K/);
...
test("the `/compact` command's own entry is never offered as a cut point", () => {
  ...
  assert.match(
    out,
    /1\. "Ordinary prompt one"\s+sent \d\d:\d\d \| valid until \d\d:\d\d \| 60K tokens before it, keeps 80K/,
  );
  assert.match(
    out,
    /2\. "Ordinary prompt two"\s+sent \d\d:\d\d \| valid until \d\d:\d\d \| 100K tokens before it, keeps 40K/,
  );
});
  ...
  assert.match(
    ...
    new RegExp(
      `1\\. "Now add the skill that takes a fresh reading"\\s+` +
        ...
        `100K tokens before it, keeps 100K`,
    ),
