...
// What that session's reading says whether or not a prompt is in flight.
...
for (const runtime of runtimes()) {
	...
	test(
		...
		() => {
			...
			// point, so the cached list is empty here too. Something was sent all
			// the same, so the reading may not infer from an empty list that
			// nothing has been.
			assert.match(
				read(
					...COMPACTED_SESSION,
	...
	test(name("a reading with no payback figure in it discloses no rate"), () => {
		...
		// rate. Nothing here is priced at that rate, since no prompt is cached and
		// no payback figure is printed. A rate disclosed over a passage that
		// quotes no number reads as a fact about the compaction beside it.
		const out = read(
			prompt("Read the brief and start on the scanner", at(120), {
				uuid: "kept-1",
			}),
