...
Price then chooses among what the arc admits: the lowest payback, and `/compact` where two of them tie. The payback is how many more requests to the model the work must make before the option has saved more than it cost. A request is one model call, and every tool call ends one and starts the next, so a reply that made ten tool calls is eleven of them and a payback of thirty is inside a step or two of agent work; carrying on, the last row, wins only where the arc has fewer requests left than every payback, an arc that is all but over, and say so then rather than recommending a cut the session will not live to earn back. The `/compact` row is priced on an estimated tail, so a request or two between it and a cut is inside that estimate. Every figure assumes the session keeps working, so all of them are pessimistic for one that pauses past the cache lifetime, where the next request rewrites the whole context either way.
