...
The arc rules on what is admissible before any figure is read: whatever the next steps still lean on has to survive verbatim. That rules `/compact` out where the summary it leaves would not hold that setup, however cheap the row looks, and it admits a rewind only at a prompt the arc began at or after, the oldest listed one that qualifies; the `context-budget` skill's "Naming the cut point" says which prompt that is. If the arc began after every listed prompt, no cut point is admissible, and the choice is `/compact [focus]` or carrying on, on the figures below.

Price then decides between the cut the arc admits and carrying on. `/compact` keeps the least, so it always pays back soonest, and a rewind pays back later the more it keeps verbatim: take `/compact` wherever the arc admits it, and a rewind only where the arc rules `/compact` out. The payback is how many more requests to the model the work must make before the option has saved more than it cost. A request is one model call, and every tool call ends one and starts the next, so a reply that made ten tool calls is eleven of them and a payback of thirty is inside a step or two of agent work. Carrying on, the last row, wins only where the arc has fewer requests left than that payback, an arc that is all but over; say so then rather than recommending a cut the session will not live to earn back. Every figure assumes the session keeps working, so all of them are pessimistic for one that pauses past the cache lifetime, where the next request rewrites the whole context either way.
